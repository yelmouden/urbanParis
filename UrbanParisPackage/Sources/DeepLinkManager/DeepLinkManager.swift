...
public struct ResetPasswordDeepLink: DeepLink {

    public static let template =  DeepLinkTemplate()
        .term("resetPassword")
        .queryStringParameters([.requiredString(named: "code")])

    public init(values: DeepLinkValues) {
        self.code = values.query["code"] as? String ?? ""
    }
...
public struct DeepLinkManager {
    public var handleDeepLink: @Sendable (_ url: URL) -> DeepLink? = { _ in nil }
}

extension DeepLinkManager: DependencyKey {
    public static var liveValue: DeepLinkManager {
        .init { url in
            let recognizer = DeepLinkRecognizer(deepLinkTypes: [ResetPasswordDeepLink.self])

            if let deepLink = recognizer.deepLink(matching: url) as? ResetPasswordDeepLink {
                return deepLink
            }
...
public extension DependencyValues {
  var deepLinkManager: DeepLinkManager {
    get { self[DeepLinkManager.self] }
    set { self[DeepLinkManager.self] = newValue }
  }
}
